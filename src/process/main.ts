...
export default class ChildProcess extends Process {
    ...
    public async initialize(): Promise<void> {
        ...
        const response: DataResponse = await this.requestExternal('aarontburn.Monkey_Core', 'add-window', {
            ...
            filter: (w: Window) => w.path.endsWith('Slack.exe') && w.isVisible(),
            callback: this.onMonkeyEvent.bind(this)
        } as MonkeyParams);

        if (response.code === HTTPStatusCodes.NOT_FOUND) {
            console.error(`[Slack Monkey] Missing dependency: Monkey Core (aarontburn.Monkey_Core) https://github.com/aarontburn/nexus-monkey-core`);
    ...
    public registerSettings(): (Setting<unknown> | string)[] {
        return [
            new StringSetting(this)
                .setDefault('')
                .setName("Slack Executable Path")
                .setDescription("The path to your Slack executable file. Restart required.")
                .setAccessID('path')
                .setValidator(s => {
            ...
            new BooleanSetting(this)
                .setName("Close Slack on Exit")
                .setDefault(false)
                .setDescription("This will only work when Slack is opened through Slack Monkey. Restart required.")
